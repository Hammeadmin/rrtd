...
import { Loader2, Info, ListFilter, ChevronUp, ChevronDown, Archive, XCircle, Building2 as EmployerIcon, Briefcase, CalendarCheck, FileText, Download, TrendingUp, TrendingDown, Wallet, ClipboardList } from 'lucide-react';
...
interface Payslip {
    ...
    net_pay: number;
    processed_at: string;
    source_record_ids: string[]; // <-- ADD THIS LINE
}
...
const PayslipCard: React.FC<{ payslip: Payslip }> = ({ payslip }) => {
    const handleDownload = () => {
        toast.info("PDF-nedladdning är inte implementerad än.");
    };

    return (
        <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden transform hover:scale-[1.02] transition-transform duration-300">
            <div className="p-5 bg-gradient-to-r from-primary-500 to-primary-600 text-white">
                ...
                <p className="text-xs opacity-80 mt-2">Bearbetad: {formatDateSafe(payslip.processed_at, 'PPP p', sv)}</p>
            </div>
            <div className="p-5 space-y-3 bg-gray-50">
                 <div className="flex justify-between items-center text-sm">
                    <span className="flex items-center text-gray-600"><TrendingUp size={16} className="mr-2 text-green-500"/>Bruttolön</span>
                    <span className="font-medium text-gray-800">{payslip.gross_pay.toFixed(2)} SEK</span>
                </div>
                 <div className="flex justify-between items-center text-sm">
                    <span className="flex items-center text-gray-600"><Briefcase size={16} className="mr-2 text-green-500"/>Semesterersättning</span>
                    <span className="font-medium text-green-600">+ {payslip.vacation_pay_added.toFixed(2)} SEK</span>
                </div>
                <div className="flex justify-between items-center text-sm">
                    <span className="flex items-center text-gray-600"><TrendingDown size={16} className="mr-2 text-red-500"/>Preliminär skatt</span>
                    <span className="font-medium text-red-500">- {payslip.tax_deducted.toFixed(2)} SEK</span>
                </div>
            </div>
            <div className="p-3 bg-white border-t flex justify-end">
                 <button onClick={handleDownload} className="btn btn-secondary btn-sm">
                    <Download size={14} className="mr-1.5"/> Ladda ner PDF
                </button>
            </div>
        </div>
    );
...
const MyPayrollPage: React.FC = () => {
  ...
  const [payrollInfo, setPayrollInfo] = useState(null);
    const [isLoadingInfo, setIsLoadingInfo] = useState(true); // For the info check
  ...
  const loadAllMyData = useCallback(async () => {
    ...
    try {
        const [payslipsRes, rawRecordsRes] = await Promise.all([
            supabase.from('payslips').select('*').eq('employee_id', profile.id).order('pay_period', { ascending: false }),
            fetchMyPayrollRecords() // Your existing function from lib/payroll
        ]);

        if (payslipsRes.error) throw payslipsRes.error;
        if (rawRecordsRes.error) throw new Error(rawRecordsRes.error as string);

        const fetchedPayslips = payslipsRes.data || [];
        setPayslips(fetchedPayslips);

        const processedRecordIds = new Set(
            fetchedPayslips.flatMap(p => p.source_record_ids || [])
        );
        const pending = (rawRecordsRes.data || []).filter(
            rec => !processedRecordIds.has(rec.id)
        );
        setUnprocessedRecords(pending);
  ...
  useEffect(() => {
   ...
   if (authLoading || isLoadingInfo) return <div className='fixed inset-0 flex items-center justify-center'><Loader2 className="h-12 w-12 animate-spin text-blue-600" /></div>;
...
return (
    <div className="p-4 md:p-8 bg-slate-50 min-h-screen">
        ...
        <main className="space-y-12">
            {!payrollInfo ? (
                // This is where you render your onboarding form component
                ...
                <>
                        {/* SECTION 1: FINAL PAYSLIPS */}
                        <div>
                            <h2 className="text-2xl font-semibold text-slate-700 mb-4">Mina Lönebesked</h2>
                            {payslips.length > 0 ? (
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                 ...
                                 <div className="text-center py-12 bg-white rounded-lg shadow-md border">
                                    ...
                                    <h3 className="text-lg font-medium">Inga bearbetade lönebesked</h3>
                                 </div>
                            )}
                        </div>
