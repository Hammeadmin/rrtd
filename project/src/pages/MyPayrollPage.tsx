...
const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
  // Fetch payroll_information for the current user
  const fetchPayrollInfo = async () => {
    const { data } = await supabase.from('payroll_information').select('*').eq('id', profile.id).single();
    setPayrollInfo(data);
    setLoading(false);
  };
  fetchPayrollInfo();
...
if (!payrollInfo) {
  // If no payroll info exists, show the onboarding form
  return <PayrollOnboardingForm userId={profile.id} onComplete={() => fetchPayrollInfo()} />;
} else {
  // If info exists, show their payroll history
  return <MyPayrollHistory payrollInfo={payrollInfo} />;
}
