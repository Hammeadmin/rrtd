...
const handleExportCSV = async () => {
    ...
    try {
        ...
        const { data: payslips, error } = await supabase
            ...
            .or(`source_record_ids.cs.{${recordIds.join(',')}}`);
  ...
  const handleOpenProcessModal = () => {
        ...
        if (records.length === 0) {
            toast.info("Inga väntande poster valda att bearbeta.");
            return;
        }
...
const handleCloseAdjustmentsModal = (refresh: boolean = true) => {
  ...
  const handleGeneratePayslipsPDF = async () => {
    ...
    if (processedRecordIds.length === 0) {
        return toast.info("Välj en eller flera 'Bearbetade' poster för att ladda ner lönebesked.");
    }

    toast.loading("Hämtar lönebesked och skapar PDF...", { id: toastId });

    try {
        // Fetch the official payslips that contain these record IDs
        const { data: payslips, error } = await supabase
            .from('payslips')
            .select('*, employee:employee_id(full_name)')
            .or(`source_record_ids.cs.{${processedRecordIds.join(',')}}`); // Fetches any payslip containing any of the selected IDs
        ...
        payslips.forEach((payslip, index) => {
            ...
            doc.text(`Anställd:`, 14, yPos); doc.text((payslip.employee as any)?.full_name || 'Okänd', 50, yPos); yPos += 7;
            doc.text(`Löneperiod:`, 14, yPos); doc.text(formatDateSafe(payslip.pay_period, 'MMMM yyyy', sv), 50, yPos); yPos += 12;

            doc.setFontSize(12); doc.setFont('helvetica', 'bold'); doc.text('Summering', 14, yPos); yPos += 8;

            const addSummaryRow = (label: string, value: string, isBold: boolean = false, color: 'normal' | 'green' | 'red' = 'normal') => {
                doc.setFontSize(10); doc.setFont('helvetica', isBold ? 'bold' : 'normal');
                if(color === 'green') doc.setTextColor(0, 128, 0);
                else if (color === 'red') doc.setTextColor(200, 0, 0);
            ...
            addSummaryRow('Bruttolön (från pass/uppdrag):', `${payslip.gross_pay.toFixed(2)} SEK`);
            if (payslip.vacation_pay_added > 0) {
                addSummaryRow('Semesterersättning:', `+ ${payslip.vacation_pay_added.toFixed(2)} SEK`, false, 'green');
            }
             addSummaryRow('Preliminär skatt:', `- ${payslip.tax_deducted.toFixed(2)} SEK`, false, 'red');

            yPos += 3; doc.setDrawColor(180, 180, 180); doc.line(14, yPos, pageWidth - 14, yPos); yPos += 8;

            doc.setFontSize(14);
            addSummaryRow('Nettolön att betala:', `${payslip.net_pay.toFixed(2)} SEK`, true);
...
const fetchEmailLogs = useCallback(async (period?: string) => {
    ...
    const handleBulkUpdateStatus = async (newStatus: 'paid', idsToUpdate: string[]) => {
        if (idsToUpdate.length === 0) {
            toast.info(`Inga poster att markera som '${newStatus}'.`);
            return;
        }
    ...
    >
        ...
        Bearbeta ({selectedPendingCount})
                    ...
                    <button onClick={() => {
                    const idsToPay = sortedRecords
                        .filter(r => selectedRecordIds.has(r.id) && r.status === 'processed')
                        .map(r => r.id);
                    handleBulkUpdateStatus('paid', idsToPay);
                }} disabled={isLoading || selectedProcessedCount === 0} className="btn btn-success btn-sm">
                    <DollarSign size={14} className="mr-1.5" />Betala ({selectedProcessedCount})
                </button>
                </div>
            </div>
